Feature: Create a visit
  ...
  Scenario: Try to create an invalid visit without a description for the pet of an owner (Error flow)
    ...
    Then no visit with description "A visit" and date "" will exist for pet "Sprigs" of owner "Last"
    And the error "must not be empty" shall be raised
  ...
  Scenario Outline: Try to create an invalid visit with an invalid date for the pet of an owner (Normal flow)
    ...
    Then no visit with description "<description>" and date "<date>" will exist for pet "Sprigs" of owner "Last"
    And the error "<error_message>" shall be raised

    Examples:
      | description | date        | error_message |
      | A visit     | 99999-03-10 | invalid date  |
      | A visit     | 10/10/1999  | invalid date  |
